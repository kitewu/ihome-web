...
<body>
			...
			<div class="col-md-12">
				<div class="panel panel-default">
					...
					<div class="panel-footer" id="id_check_passwd">
						<div class="input-group"><input id="input_password" type="password" class="form-control input-md" placeholder="输入homeid密码以完成验证"><span class="input-group-btn"><button class="btn btn-primary btn-md" onclick="checkPassword()"> 确定 </button></span></div>
					</div>
				</div>
			</div>
		</div>
	...
	<script>
		...
		function checkPassword(){
			...
			$.ajax( {
				type : "post",
				url : './backdeal/CheckHomeIDPasswd.php',
				data : {Password : password},
				success : function(data) {
					if(data == '验证成功'){
						window.location.href='updateaccess.php';
					}else{
						alert(data);
					}
				}
